...
export const fractionToFloat = (frac) => {
...
export const formatPrecise = (percent) => {
    return (fractionToFloat(percent.asFraction) * 100).toLocaleString(undefined, {
        maximumFractionDigits: 10,
    });
};
export const FORMAT_DOLLARS = {
    currency: "USD",
    style: "currency",
};
export const FORMAT_DOLLARS_WHOLE = {
    currency: "USD",
    style: "currency",
    minimumFractionDigits: 0,
    maximumFractionDigits: 0,
};
export const formatCurrency = (amount, currency, numberFormatOptions = {}) => {
...
export const formatCurrencyWhole = (amount, currency, numberFormatOptions = {}) => {
    if (currency === CurrencyMarket.USD) {
        return amount.toLocaleString(undefined, {
...
export const formatCurrencySmart = (amount, currency, numberFormatOptions) => {
    const amtFloat = amount instanceof Fraction ||
        (typeof amount === "object" && amount?.toFixed)
        ? fractionToFloat(amount)
        : amount;
    const threshold = currency === CurrencyMarket.USD
        ? 100
 ...
 * 1.87654321 => 1.876543
 */
export const formatDisplayWithSoftLimit = (float, maxDecimals, softMaximumSignificantDigits = 7, numberFormatOptions, locale) => {
    if (Number.isNaN(softMaximumSignificantDigits) ||
    ...
    if (float >= dropDecimalsAfter) {
        ...
        const wholeNumberFormatOptions = Object.assign({}, numberFormatOptions, {
            maximumFractionDigits: 0,
        });
        return Math.floor(float).toLocaleString(locale, wholeNumberFormatOptions);
    }
    // Round to maxDecimals if too small
    const sigDerivedDigitsOnRight = softMaximumSignificantDigits - Math.floor(Math.log10(float)) - 1;
    const digitsOnRight = Math.max(0, Math.min(maxDecimals, sigDerivedDigitsOnRight));
    const flooredToPrecision = Math.floor(float * 10 ** digitsOnRight) / 10 ** digitsOnRight;
    const maxFormatOptions = Object.assign({
...
export const formatNumberSI = (value) => {
...
export const formatDurationSeconds = (seconds) => formatDuration(intervalToDuration({
    start: new Date(0),
    end: new Date(seconds * 1_000),
}), { delimiter: ", " });
export const formatSignificantDistance = (end, start) => formatDuration(intervalToDuration({
    start,
    end,
}), { delimiter: ", " })
    .split(", ")
    .slice(0, 3)
    .join(", ");
export const formatSignificantDistanceWithSuffix = (end, start) => {
