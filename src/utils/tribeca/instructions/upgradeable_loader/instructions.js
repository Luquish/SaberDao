 ...
 */
export const createUpgradeInstruction = async ({ program, buffer, spill, signer, }) => {
    const [programData] = await PublicKey.findProgramAddress([program.toBuffer()], BPF_UPGRADEABLE_LOADER_ID);
...
export const findProgramDataAddress = (programID) => PublicKey.findProgramAddress([programID.toBuffer()], BPF_UPGRADEABLE_LOADER_ID);
/**
 ...
 *      not be upgradeable.
 */
export const createSetAuthorityInstruction = ({ account, authority, nextAuthority, }) => {
    return new TransactionInstruction({
        programId: BPF_UPGRADEABLE_LOADER_ID,
 ...
 */
export const createCloseInstruction = ({ account, spill, authority, program, }) => {
    const keys = [
        {
            pubkey: account,
            isSigner: false,
