...
export class ProgramError extends Error {
    ...
    underlying;
    program;
    constructor(errorInfo, underlying, program) {
    ...
    static parse(err, txEnv, programIDs, programErrors, programNameOverride) {
        ...
        const programName = programNameOverride ??
...
const codesToName = Object.entries(LangErrorCode).reduce((acc, el) => ({
    ...acc,
    [el[1]]: el[0],
}), {});
 ...
 * @returns
 */
export const parseError = (errantProgramID, code, err) => {
    const handler = handlers[errantProgramID.toString()];
    if (handler) {
        return new ProgramError({
            code,
            displayMessage: handler.displayMessage(code) ?? `unknown (code ${code})`,
            name: code.toString(),
        }, err.toString(), handler.name);
    }
