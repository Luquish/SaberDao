import { TOKEN_ACCOUNT_PARSER, useParsedAccountData, usePubkey, useSail, useToken, useUserATAs, } from "@rockooor/sail";
 ...
 * @returns
 */
const computeRedeemableAmount = (stream) => {
...
export const VenkoLockupInner = () => {
    ...
    const { data: walletTokens } = useParsedAccountData(walletStreamATA, TOKEN_ACCOUNT_PARSER);
    const hasSmartWalletBalance = walletTokens?.accountInfo.data.amount.gt(new BN(0));
    ...
    const withdrawSmartWallet = async () => {
        ...
        const redeemableAmount = new TokenAmount(streamToken, computeRedeemableAmount(stream.accountInfo.data));
        notify({
        ...
        const { pending, success } = await handleTX(await wrapTx(newTX), "Propose: Redeem Tokens");
        if (!pending || !success) {
            return;
        }
        await pending.wait();
    ...
    const withdraw = async () => {
        ...
        const { pending, success } = await handleTX(await wrapTx(redeemTX), "Redeem Tokens");
        if (!pending || !success) {
            return;
        }
        await pending.wait();
    ...
    return (React.createElement("div", { tw: "flex flex-col gap-4" },
        React.createElement(LabeledInput, { Component: InputText, label: "Stream Mint", type: "text", placeholder: "Mint address of your Stream token.", value: mintStr, onChange: (e) => {
                setMintStr(e.target.value);
            } }),
        stream && streamToken && (React.createElement("div", null,
            React.createElement(AttributeList, { attributes: {
                    Stream: stream.accountId,
                    Mint: stream.accountInfo.data.mint,
                    Start: new Date(stream.accountInfo.data.startTs.toNumber() * 1_000),
                    Cliff: new Date(stream.accountInfo.data.cliffTs.toNumber() * 1_000),
                    End: new Date(stream.accountInfo.data.endTs.toNumber() * 1_000),
                    "Initial Amount": new TokenAmount(streamToken, stream.accountInfo.data.initialAmount),
                    "Redeemed Amount": new TokenAmount(streamToken, stream.accountInfo.data.redeemedAmount),
                    "Redeemable Amount": new TokenAmount(streamToken, computeRedeemableAmount(stream.accountInfo.data)),
                } }),
            smartWallet && walletTokens && (React.createElement("div", null,
                React.createElement("p", null,
                    "Smart Wallet Balance:",
                    " ",
                    new TokenAmount(streamToken, walletTokens.accountInfo.data.amount).formatUnits()),
                React.createElement(Button, { disabled: !hasSmartWalletBalance, variant: "primary", onClick: async () => {
                        await withdrawSmartWallet();
                    } }, "Withdraw Smart Wallet"))),
            hasStreamBalance && userStreamTokens && (React.createElement("div", null,
                React.createElement("p", null,
                    "Your balance: ",
                    userStreamTokens.balance.formatUnits()),
                React.createElement(AsyncButton, { variant: "primary", onClick: async () => {
                        await withdraw();
                    } }, "Withdraw All"))),
            !hasStreamBalance && !hasSmartWalletBalance && (React.createElement("div", null, "You don't have any stream tokens. Make sure you're on the correct Smart Wallet or connected to the right wallet."))))));
};
