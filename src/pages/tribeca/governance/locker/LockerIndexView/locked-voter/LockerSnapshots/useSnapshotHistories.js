import { calculateEra, calculateEraStart, calculatePeriodStart, ERA_NUM_PERIODS, findEscrowHistoryAddress, findLockerHistoryAddress, PERIOD_SECONDS, SNAPSHOTS_CODERS, SnapshotsSDK, } from "@saberhq/snapshots";
import { isNotNull, TransactionEnvelope, tsToDate, } from "@saberhq/solana-contrib";
...
import { useBatchedEscrowHistories, useBatchedLockerHistories, } from "@/utils/tribeca/parsers";
import { useEnvironment } from "@/utils/useEnvironment";
export const useLockerHistories = (locker, eras) => {
    const { data: lockerHistoryKeys } = useQuery({
        ...
        queryFn: async () => {
            ...
            return await Promise.all(eras.map(async (era) => {
...
export const useEscrowHistories = (escrow, eras) => {
...
export const calculatePeriod = (eraStart, date) => {
    return Math.floor((date.getTime() - eraStart.getTime()) / 1_000 / PERIOD_SECONDS);
};
export const syncSnapshots = async ({ provider, locker, escrow, eras, }) => {
    const sdk = SnapshotsSDK.load({ provider });
    const keys = await Promise.all(eras.map(async (era) => {
        const [lockerHistory] = await findLockerHistoryAddress(locker, era);
        const [escrowHistory] = await findEscrowHistoryAddress(escrow, era);
        return { lockerHistory, escrowHistory };
    }));
    return zip(eras, keys).map(([era, lockerAndEscrow]) => {
 ...
 */
export const createAndSyncSnapshots = async ({ provider, refetchMany, locker, escrow, eras, }) => {
    const sdk = SnapshotsSDK.load({ provider });
    const keys = await Promise.all(eras.map(async (era) => {
        const [lockerHistory] = await findLockerHistoryAddress(locker, era);
        const [escrowHistory] = await findEscrowHistoryAddress(escrow, era);
    ...
    const fetched = await refetchMany(keys.flatMap((k) => [k.lockerHistory, k.escrowHistory]));
    const result = zip(eras, keys, chunk(fetched, 2)).map(([era, lockerAndEscrow, histories]) => {
        invariant(typeof era === "number");
        ...
        if (lockerHistoryData === null) {
            ixs.push(sdk.snapshots.program.instruction.createLockerHistory(era, {
                accounts: {
                    locker,
                    lockerHistory,
        ...
        if (escrowHistoryData === null) {
            ixs.push(sdk.snapshots.program.instruction.createEscrowHistory(era, {
                accounts: {
                    escrow,
                    escrowHistory,
                    payer: provider.walletKey,
...
export const useSnapshotHistories = (owner) => {
    ...
    const eras = lastEra !== null
        ...
        : erasToFetch;
    const { data: lockerHistories } = useLockerHistories(lockerData ? lockerData.publicKey : lockerData, eras);
    ...
    const unsyncedSnapshots = useMemo(() => {
        ...
        const erasToSync = eras.map((era, i) => {
            ...
            const { maxStakeDuration, maxStakeVoteMultiplier } = lockerData.account.params;
            const firstPeriodStart = calculatePeriodStart(era, firstPeriod);
            const lastPeriodStart = calculatePeriodStart(era, lastPeriod);
            const firstDuration = Math.floor((firstPeriodStart.getTime() - escrowStart.getTime()) / 1_000);
            ...
            const lastDuration = Math.floor((lastPeriodStart.getTime() - escrowStart.getTime()) / 1_000);
    ...
    const { data: lockerHistoryCost } = useMinBalanceRentExempt(lockerHistorySize);
    const { data: escrowHistoryCost } = useMinBalanceRentExempt(escrowHistorySize);
    const { network } = useEnvironment();
    const estimatedCost = useMemo(() => {
        if (missingLockerHistoriesCount === undefined ||
            missingEscrowHistoriesCount === undefined) {
            return undefined;
        }
        const sol = RAW_SOL[network];
        let total = new TokenAmount(sol, 0);
        if (lockerHistoryCost && missingLockerHistoriesCount) {
            total = total.add(new TokenAmount(sol, JSBI.multiply(lockerHistoryCost.raw, JSBI.BigInt(missingLockerHistoriesCount))));
        }
        if (escrowHistoryCost && missingEscrowHistoriesCount) {
            total = total.add(new TokenAmount(sol, JSBI.multiply(escrowHistoryCost.raw, JSBI.BigInt(missingEscrowHistoriesCount))));
        }
        return total;
    }, [
        escrowHistoryCost,
