import { findGaugeAddress, findGaugeVoteAddress, findGaugeVoterAddress, } from "@quarryprotocol/gauge";
...
export const GaugeWeightRow = ({ quarry }) => {
    ...
    const { currentTotalShares, nextShares, nextTotalShares, nextSharesRaw, setTokenShareStr, } = useUpdateGaugeWeights();
    ...
    useEffect(() => {
        if (currentShare) {
            setTokenShareStr(quarry.quarry.account.tokenMintKey, currentShare, currentShare.toString());
        }
    }, [
        currentShare,
    ...
    return (React.createElement("tr", null,
        ...
        React.createElement("td", null, currentSharePercent !== null
            ? FORMAT_VOTE_PERCENT.format(currentSharePercent)
            : "--"),
        React.createElement("td", null,
            React.createElement(InputText, { value: nextSharesRaw[quarry.quarry.account.tokenMintKey.toString()]
                    ?.value ?? "", onChange: (e) => {
                    setTokenShareStr(quarry.quarry.account.tokenMintKey, currentShare ?? 0, e.target.value);
                } })),
        React.createElement("td", null, percent !== null ? FORMAT_VOTE_PERCENT.format(percent) : "--")));
};
