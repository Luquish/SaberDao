import { findEpochGaugeVoterAddress, findGaugeAddress, findGaugeVoteAddress, findGaugeVoterAddress, GAUGE_CODERS, GaugeSDK, } from "@quarryprotocol/gauge";
...
export const SetWeightsModal = () => {
    ...
    const makePlan = useCallback(async () => {
        ...
        const gaugeVoteKeys = await Promise.all(gaugeKeys.map(async (gaugeKey) => {
            const [gaugeVoteKey] = await findGaugeVoteAddress(gaugeVoterKey, gaugeKey);
            return gaugeVoteKey;
        }));
        const gaugeVotes = await refetchMany(gaugeVoteKeys);
        const parsedGaugeVotes = gaugeVotes.map((gv) => gv && "data" in gv
            ? GAUGE_CODERS.Gauge.accounts.gaugeVote.parse(gv.data)
            : null);
        const disabledGaugesToReset = gauges
            ?.filter((gauge) => !!gauge?.account.isDisabled &&
            !!parsedGaugeVotes.find((gv) => gv?.gauge.equals(gauge.publicKey) && gv.weight !== 0))
            .map((gauge) => ({
        ...
        const shareUpdates = await gauge.gauge.setVotes({
            ...
            weights: [
                ...
                ...(await Promise.all(sharesDiff.map(async (diff) => {
                    invariant(diff.nextShareParsed !== null, "next share parsed");
                    const [gaugeKey] = await findGaugeAddress(gaugemeister, diff.quarry.publicKey);
                    return {
                        gauge: gaugeKey,
        ...
        const gaugesDiffed = await Promise.all(sharesDiff.map(async (diff) => {
            const [gaugeKey] = await findGaugeAddress(gaugemeister, diff.quarry.publicKey);
        ...
        const gaugesToCommit = gaugesToUpdate.filter((gk) => {
            ...
            !gauges?.find((gauge) => gauge?.publicKey.equals(gk) && gauge?.account.isDisabled) &&
                gaugeKeys.find((gaugeKey, i) => {
                    if (!gk.equals(gaugeKey)) {
                        return false;
                    }
                    // get the new vote
                    const nextVote = gaugesDiffed.find((gd) => gd.gaugeKey.equals(gaugeKey));
        ...
        const [escrow] = await findEscrowAddress(gmData.account.locker, sdkMut.provider.wallet.publicKey);
        const [gaugeVoter] = await findGaugeVoterAddress(gaugemeister, escrow);
        const [epochGaugeVoter] = await findEpochGaugeVoterAddress(gaugeVoter, gmData.account.currentRewardsEpoch + 1);
        const epochGaugeVoterData = await gauge.gauge.fetchEpochGaugeVoter(epochGaugeVoter);
    ...
    const allTokens = useTokens(sharesDiff.map((share) => share.quarryInfo.quarry.account.tokenMintKey));
    return (React.createElement(ModalInner, { title: "Update Weights" },
        React.createElement("div", { tw: "mb-4" },
            React.createElement(AttributeList, { transformLabel: false, attributes: sharesDiff.reduce((acc, el) => ({
                    ...acc,
                    [allTokens.find((token) => token.data?.mintAccount.equals(el.quarryInfo.quarry.account.tokenMintKey))?.data?.name ??
                        el.quarryInfo.quarry.account.tokenMintKey.toString()]: `${el.prevShareParsed ?? "(null)"} -> ${el.nextShareParsed ?? "(null)"}`,
                }), {}) })),
        React.createElement(TransactionPlanExecutor, { makePlan: makePlan, onComplete: close })));
};
