...
export const useGaugeKeys = () => {
    ...
    const { data: gaugeKeys } = useQuery({
        ...
        queryFn: async () => {
            ...
            const keys = await Promise.all(allStakedTokenMints.map(async (stakedTokenMin) => {
                const [quarryKey] = await findQuarryAddress(rewarderKey, stakedTokenMin);
...
export const useGauge = (stakedTokenMint) => {
