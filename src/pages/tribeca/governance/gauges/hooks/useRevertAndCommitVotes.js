import { findEpochGaugeVoterAddress, findGaugeVoteAddress, findGaugeVoterAddress, GAUGE_CODERS, GaugeSDK, } from "@quarryprotocol/gauge";
...
export const useRevertAndCommitVotes = () => {
    ...
    return useCallback(async () => {
        ...
        await signAndConfirmTXs(await wrapTx(TransactionEnvelope.pack(...revertTXs)), "Revert votes");
        ...
        if (epochGaugeVoterData) {
            if (!epochGaugeVoterData.allocatedPower.isZero()) {
                ...
                await signAndConfirmTX(await wrapTx(await gauge.gauge.resetEpochGaugeVoter({
                    gaugemeister,
                })), "Reset committed votes");
            }
        }
        else {
            await signAndConfirmTX(await wrapTx(await gauge.gauge.prepareEpochGaugeVoter({
                gaugemeister,
            })), "Prepare votes");
        }
        ...
        await signAndConfirmTXs(await wrapTx(TransactionEnvelope.pack(...voteTXs)), "Commit votes");
    }, [
        gaugeKeys,
        gaugemeister,
        gmData,
