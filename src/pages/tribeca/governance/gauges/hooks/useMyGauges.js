...
export const useVoterGauges = (escrowKey) => {
    ...
    const { data: gaugeVoteKeys } = useQuery({
        ...
        queryFn: async () => {
            ...
            const gaugeVoteKeys = await Promise.all(gaugeKeys.map(async (gauge) => {
    ...
    const { data: gaugeVoter } = useGaugeVoterData(mapSome(gaugeVoteKeys, (x) => x.gaugeVoterKey));
    const { data: gaugeVotes } = useBatchedGaugeVotes(mapSome(gaugeVoteKeys, (x) => x.gaugeVoteKeys));
    const totalWeight = gaugeVoter ? gaugeVoter.account.totalWeight : gaugeVoter;
    const myGauges = useMemo(() => gaugeVotes
        ?.filter((gv) => !!gv)
        .map((gv) => {
        const weight = gv.account.weight;
        const percent = typeof totalWeight === "number" ? weight / totalWeight : null;
        return {
            key: gv.publicKey,
            ...gv.account,
            percent,
        ...
        .sort((a, b) => (a.weight > b.weight ? -1 : 1)), [gaugeVotes, totalWeight]);
    return {
        gaugeVoter,
        gaugeVotes,
        myGauges,
        hasNoGauges: !gaugeVotes ||
