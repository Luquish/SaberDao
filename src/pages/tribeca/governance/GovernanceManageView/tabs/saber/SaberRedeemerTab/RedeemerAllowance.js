...
export const RedeemerAllowance = ({ redeemer }) => {
    ...
    const { data: minterInfoKey } = useQuery({
        ...
        queryFn: async () => {
            const minterInfo = await associated(SABER_ADDRESSES.MintProxy, redeemer.toBuffer());
            return minterInfo;
        },
    });
    ...
    return (React.createElement(Card, { title: "Increase Allowance" },
        React.createElement(AttributeList, { attributes: {
                Minter: redeemer,
                "Minter Info": minterInfoKey,
                "Initial Amount": initialAmount,
                "Allowance Remaining": token && minterInfo
                    ? new TokenAmount(token, minterInfo.account.allowance)
                    : null,
            } }),
        minterInfo ? (React.createElement(AsyncButton, { className: "mt-6", disabled: !token || !initialAmount || !ownerInvokerKey, onClick: async (sdkMut) => {
                invariant(ownerInvokerKey);
                invariant(token && initialAmount, "token");
                const saberSDK = Saber.load({ provider: sdkMut.provider });
                const minterInfo = await saberSDK.mintProxy.program.account.minterInfo.associatedAddress(redeemer);
                const minterUpdateTX = saberSDK.mintProxy.program.state.instruction.minterUpdate(initialAmount.toU64(), {
                    accounts: {
                        auth: { owner: ownerInvokerKey },
                        minterInfo,
                    },
                });
                await ownerInvokeTX(sdkMut.provider.newTX([minterUpdateTX]), `Update minter`);
            } }, `Set Allowance to ${initialAmount?.formatUnits() ?? "--"}`)) : (React.createElement(AsyncButton, { className: "mt-6", onClick: async (sdkMut) => {
                invariant(initialAmount, "initial amount");
                const saber = Saber.load({ provider: sdkMut.provider });
                const minterAddTX = await saber.mintProxy.minterAdd(redeemer, initialAmount.toU64());
