...
export const CreateGaugesModal = () => {
    const { sdkMut } = useSDK();
    ...
    const create = async () => {
        ...
        const allGaugeKeys = await Promise.all(quarries?.map(async (quarry) => {
            const [gaugeKey] = await findGaugeAddress(gaugemeister, quarry.key);
            return gaugeKey;
        }) ?? []);
        const gaugesData = await refetchMany(allGaugeKeys);
        const createTXs = await Promise.all(quarries?.map(async (quarry, i) => {
        ...
        const bigEnvelope = TransactionEnvelope.combineAll(...createTXs.filter((tx) => !!tx));
        const { pending, success } = await handleTXs(await wrapTx(bigEnvelope.partition()), `Create ${createTXs.length} Gauges`);
        if (!success) {
            return;
        }
    ...
    return (React.createElement(ModalInner, { title: "Create Gauges", buttonProps: {
            onClick: create,
            variant: "primary",
            children: "Create Gauges",
        } },
        React.createElement("div", { tw: "px-8 flex flex-col items-center" },
            React.createElement("p", { tw: "mb-4 text-sm" }, "Create Gauges for missing Quarries."))));
};
