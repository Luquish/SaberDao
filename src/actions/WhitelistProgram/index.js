...
export const WhitelistProgram = ({ actor, ctx, payer, setTxRaw, }) => {
    ...
    const { data: whitelistEntryKey } = useQuery({
        ...
        queryFn: async () => {
            ...
            const [whitelistEntry] = await findWhitelistAddress(lockerKey, programID, null);
    ...
    useEffect(() => {
        ...
        void (async () => {
            const [whitelistEntry, bump] = await findWhitelistAddress(lockerKey, programID, null);
