...
import { TOKEN_ACCOUNT_PARSER, useAccountData, useParsedAccountData, usePubkey, useSail, } from "@rockooor/sail";
...
export const IssueTokensAction = ({ actor, ctx, setError, setTxRaw, }) => {
    ...
    const { data: iouATA } = useParsedAccountData(iouATAKey, TOKEN_ACCOUNT_PARSER);
    ...
    const { data: outputATA } = useParsedAccountData(outputATAKey, TOKEN_ACCOUNT_PARSER);
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(InputTokenAmount, { label: "Amount", token: govToken ?? null, tokens: [], tw: "h-auto", inputValue: amountStr, inputDisabled: !actor, inputOnChange: (e) => {
                setAmountStr(e);
            } }),
        React.createElement("label", { tw: "flex flex-col gap-1", htmlFor: "destination" },
            React.createElement("span", { tw: "text-sm" }, "Recipient"),
        ...
        ((iouATAKey && iouATA === null) ||
            ...
            (recipientATAKey && recipientATA === null)) && (React.createElement(AsyncButton, { disabled: !mintWrapperData || !actor || !outputToken, onClick: async (sdkMut) => {
