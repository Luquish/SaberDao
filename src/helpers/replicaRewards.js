...
const findPDASync = (seeds, programId) => {
    const [addr] = PublicKey.findProgramAddressSync(seeds.map((seed) => {
...
export const findMergePoolAddress = ({ programId = QUARRY_ADDRESSES.MergeMine, primaryMint, }) => {
    return findPDASync(['MergePool', primaryMint], programId);
};
const findReplicaMintAddress = ({ programId = QUARRY_ADDRESSES.MergeMine, primaryMint, }) => {
    const pool = findMergePoolAddress({ programId, primaryMint });
    return findPDASync(['ReplicaMint', pool], programId);
};
export const getReplicaRewards = async (quarry, lpToken, replicaQuarryInfo, wallet) => {
