...
export const useStableSwapTokens = (pool) => {
    ...
    const result = useMemo(() => {
        ...
        const underlyingTokens = tokens?.map((tok) => {
            if (tok.info.tags?.includes(Tags.DecimalWrapped)) {
                const realTok = allTokens.tokens.find((t) => t.address === tok.info.extensions?.assetContract);
                if (!realTok) {
        ...
        const newUnderlyingTokens = underlyingTokens?.filter((tok) => !tokens?.find((t) => t.address === tok?.address));
        const wrappedTokens = tokens?.map((tok, i) => {
