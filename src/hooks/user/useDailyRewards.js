...
export default function useDailyRewards(lpToken) {
    ...
    useEffect(() => {
        ...
        const rewardsT0 = new TokenAmount(new Token(SBR_INFO), payroll.calculateRewardsEarned(new BN(timeInSecT0), miner.stakedBalance, miner.data.rewardsPerTokenPaid, miner.data.rewardsEarned));
        const timeInSecT1 = Math.floor(Date.now() / 1000) + 86400;
        const rewardsT1 = new TokenAmount(new Token(SBR_INFO), payroll.calculateRewardsEarned(new BN(timeInSecT1), miner.stakedBalance, miner.data.rewardsPerTokenPaid, miner.data.rewardsEarned));
