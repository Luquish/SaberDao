...
export default function useClaim(lpToken) {
    ...
    const claimableRewards = useMemo(() => () => {
        ...
        let rewards = new TokenAmount(new Token(SBR_INFO), payroll.calculateRewardsEarned(new BN(timeInSec), miner.stakedBalanceLegacy, miner.data.rewardsPerTokenPaid, miner.data.rewardsEarned));
        if (miner.mergeMinerData && secondaryPayrolls.length === 0) {
            rewards = rewards.add(new TokenAmount(new Token(SBR_INFO), payroll.calculateRewardsEarned(new BN(timeInSec), miner.stakedBalanceMM, miner.mergeMinerData.rewardsPerTokenPaid, miner.mergeMinerData.rewardsEarned)));
        }
        // Secondary
        const secondaryRewards = secondaryPayrolls.map((secondaryPayroll) => {
            ...
            rewards = rewards.add(new TokenAmount(new Token(SBR_INFO), payroll.calculateRewardsEarned(new BN(timeInSec), secondaryPayroll.primaryMinerData.balance, secondaryPayroll.primaryMinerData.rewardsPerTokenPaid, secondaryPayroll.primaryMinerData.rewardsEarned)));
            return (new TokenAmount(secondaryPayroll.rewardsToken, secondaryPayroll.payroll.calculateRewardsEarned(new BN(timeInSec), secondaryPayroll.replicaMinerData.balance, secondaryPayroll.replicaMinerData.rewardsPerTokenPaid, secondaryPayroll.replicaMinerData.rewardsEarned))).asNumber;
        });
