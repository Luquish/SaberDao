...
import { Fraction, getOrCreateATAs, NATIVE_MINT, Percent, TokenAmount, ZERO, } from '@saberhq/token-utils';
...
export const useDeposit = ({ tokenAmounts, pool }) => {
    ...
    const estimatedMint = useMemo(() => {
        ...
        try {
            return calculateEstimatedMintAmount(pool.exchangeInfo, amountA?.raw ?? ZERO, amountB?.raw ?? ZERO);
        }
        catch (e) {
    ...
    const priceImpact = useMemo(() => {
        ...
        const totalTokens = tokenAmountsWrapped.reduce((acc, amt) => acc.add(amt.asFraction), new Fraction(0));
    ...
    const estimatedDepositSlippage = useMemo(() => {
        ...
        const [amountA, amountB] = tokenAmountsWrapped;
        return calculateDepositSlippage(pool.exchangeInfo, amountA?.raw ?? ZERO, amountB?.raw ?? ZERO);
    }, [pool.exchangeInfo, tokenAmountsWrapped]);
            ...
            : swap.state.isPaused
                ...
                : tokenAmounts.every((amount) => amount.isZero()) ||
                    ...
                    : estimatedDepositSlippage?.greaterThan(maxSlippagePercent)
                        ...
                        : undefined;
    const handleSolDeposit = useCallback(async (swap, exchangeInfo, mints, 
    ...
    noStake) => {
        ...
        try {
            const estimatedMint = calculateEstimatedMintAmount(exchangeInfo, amountA.raw, amountB.raw);
        ...
        allInstructions.push(swap.deposit({
        ...
        const txEnv = saber.newTx(allInstructions, [
            ephemeralAccount,
        ]);
    ...
    const handleDeposit = useCallback(async (noStake) => {
        ...
        if (mints.tokenA.equals(NATIVE_MINT) ||
        ...
        if (!amountA.isZero() && !amountA.token.equals(amountAInput.token)) {
            const aWrapped = await WrappedTokenActions.loadWithActions(saber.provider, SABER_CODERS.AddDecimals.getProgram(saber.provider), amountAInput.token, amountA.token.decimals);
        ...
        if (!amountB.isZero() && !amountB.token.equals(amountBInput.token)) {
            const bWrapped = await WrappedTokenActions.loadWithActions(saber.provider, SABER_CODERS.AddDecimals.getProgram(saber.provider), amountBInput.token, amountB.token.decimals);
        ...
        try {
            const estimatedMint = calculateEstimatedMintAmount(pool.exchangeInfo, amountA.raw, amountB.raw);
            // minimum lp token amount to receive from the deposit, considering slippage
            minimumPoolTokenAmount = estimatedMint.mintAmount.reduceBy(maxSlippagePercent);
        }
        catch (e) {
