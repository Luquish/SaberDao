...
const calculatePricePerLPToken = (exchange) => {
 ...
 * @returns
 */
export const calculateDepositPriceImpact = (exchange, amountA, amountB) => {
    ...
    const newLpTotalSupply = exchange.lpTotalSupply.add(new TokenAmount(exchange.lpTotalSupply.token, mintAmount.raw));
    const exchangeAfterDeposit = {
        ...
        reserves: [
            {
                ...exchange.reserves[0],
                amount: new TokenAmount(exchange.reserves[0].amount.token, JSBI.add(exchange.reserves[0].amount.raw, amountA)),
            },
            {
                ...exchange.reserves[1],
                amount: new TokenAmount(exchange.reserves[1].amount.token, JSBI.add(exchange.reserves[1].amount.raw, amountB)),
            },
        ],
    };
