 ...
 * @returns
 */
export const calculateWithdrawOne = ({ exchangeInfo, poolTokenAmount, withdrawToken, virtualPrice, maxSlippagePercent, }) => {
    const tokens = exchangeInfo.reserves.map((r) => r.amount.token);
    // Withdraw one
    ...
    const totalFee = withdrawOneAmount
        ? new TokenAmount(withdrawToken.value, JSBI.add(withdrawOneAmount.swapFee.raw, withdrawOneAmount.withdrawFee.raw))
        : undefined;
    const withdrawTokenValue = withdrawToken.value;
    const renderedFee = totalFee && !totalFee.isZero() ? totalFee : undefined;
    const tokenCalcs = tokens?.map((tok) => tok.equals(withdrawTokenValue)
        ? [withdrawOneAmount?.withdrawAmount, renderedFee]
        : [undefined, undefined]) ?? [];
    const estimates = tokenCalcs.map((c) => c[0]);
    const fees = tokenCalcs.map((c) => c[1]);
 ...
 * @returns
 */
const calculateExpectedWithdrawOneAmount = (virtualPrice, tokens, withdrawPoolTokenAmount) => {
