import { SABER_CODERS, SBR_REWARDER, WrappedTokenActions } from '@saberhq/saber-periphery';
...
const getWithdrawIxs = async (provider, saber, wallet, userAta, pool, withdrawToken, minimums, wrappedTokens, withdrawPoolTokenAmount) => {
    ...
    if (withdrawToken !== undefined) {
        ...
        const txEnv = await saber.router
            ...
            minimumAmountOut: new TokenAmount(withdrawToken.isWrapped()
                ? withdrawToken.value
                : withdrawToken.underlying, minimum),
        ...
        const { accounts: { tokenA: userAccountA, tokenB: userAccountB }, instructions, } = await getOrCreateATAs({
            provider,
            mints: {
        ...
        const allSigners = [];
        allInstructions.push(swap.withdraw({
        ...
        await Promise.all(wrappedTokens.map(async (wTok) => {
            if (wTok.isWrapped()) {
                const action = await WrappedTokenActions.loadWithActions(provider, SABER_CODERS.AddDecimals.getProgram(provider), wTok.underlying, wTok.value.decimals);
...
export const useWithdraw = ({ withdrawPoolTokenAmount, withdrawToken, wrappedTokens, pool, actions, }) => {
    ...
    const handleWithdraw = async () => {
            ...
            if (miner.mergeMiner && miner.replicaInfo) {
                ...
                await Promise.all(miner.replicaInfo.replicaQuarries.map(async (replica) => {
                    ...
                    try {
                        ...
                        const ixs = await mergePool.unstakeAllReplica(new PublicKey(replica.rewarder), mmAddress);
                ...
                const replicaDepositTxs = [];
                if (withdrawPoolTokenAmount.raw.toString() !== miner.stakedBalance.toString()) {
                    // Need restake replica
                    replicaDepositTxs.push(...(await Promise.all(miner.replicaInfo.replicaQuarries.map(async (replica) => {
                        invariant(miner.mergeMiner, 'merge miner');
                        return await mergePool.stakeReplicaMiner(new PublicKey(replica.rewarder), mmAddress);
                    }))));
                    allTxsToExecute.push({
        ...
        if (actions.withdraw) {
            invariant(userLP);
            allTxsToExecute.push(await getWithdrawIxs(provider, saber, wallet, userLP.userAta, pool, withdrawToken, minimums, wrappedTokens, withdrawPoolTokenAmount));
        }
