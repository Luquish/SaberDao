...
export const useGovernorInfo = () => {
    ...
    const governorMeta = useMemo(() => governorMetas?.find((gov) => gov.address.toString() === governorStr || gov.slug === governorStr) ?? null, [governorMetas, governorStr]);
    const slug = governorMeta?.slug ?? governorStr;
    const { data: manifest, isLoading: mfIsLoading, isFetched: mfIsFetched, } = useGovernanceManifest(slug);
...
const useGovernorInner = () => {
    ...
    const governorData = govDataRaw === undefined ? undefined : parsedGovernorData;
    const { data: lockerData } = useLockerData(governorData ? governorData.account.electorate : governorData);
    const { data: backupGovToken } = useToken(lockerData ? lockerData.account.tokenMint : lockerData);
    ...
    const minActivationThreshold = veToken && lockerData
        ? new TokenAmount(veToken, lockerData.account.params.proposalActivationMinVotes)
        : null;
    const lockedSupply = govToken && lockerData
    ...
    const governorW = useMemo(() => (tribecaMut ? new GovernorWrapper(tribecaMut, governor) : null), [governor, tribecaMut]);
...
export const { useContainer: useGovernor, Provider: GovernorProvider } = createContainer(useGovernorInner);
export const useGovWindowTitle = (title) => {
