...
export const useParsedTX = (smartWalletKey, index) => {
    const { data: key } = useTXAddress(smartWalletKey, index);
    return useParsedTXByKey(key);
};
export const useParsedTXByKey = (key) => {
    const { network } = useEnvironment();
    const { data: txData, isLoading: loading } = useGokiTransactionData(key);
    const idls = useIDLs(txData?.account.instructions.map((ix) => ix.programId) ?? []);
    const query = useQuery({
        queryKey: ["parsedTX", network, key],
        queryFn: () => {
            invariant(txData);
            const instructions = txData.account.instructions
                .map((rawIx) => ({
                ...rawIx,
                data: Buffer.from(rawIx.data),
            }))
                .map((ix) => {
                const idl = idls.find((theIDL) => theIDL.data?.programID.equals(ix.programId))?.data?.idl;
