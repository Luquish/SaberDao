...
export const useEscrow = (owner) => {
    ...
    const result = useQuery({
        ...
        queryFn: async () => {
            ...
            const escrowW = new VoteEscrow(tribecaMut, lockerKey, governor, escrow.publicKey, owner);
            return {
    ...
    const veBalance = veToken && result.data
        ? new TokenAmount(veToken, result.data.calculateVotingPower(new Date().getTime() / 1_000))
        : null;
