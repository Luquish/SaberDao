...
const programDataParser = (info) => {
    const lastDeployedSlot = u64.fromBuffer(info.accountInfo.data.slice(4, 4 + 8));
    const upgradeAuthority = new PublicKey(info.accountInfo.data.slice(4 + 8 + 1, 4 + 8 + 1 + 32));
...
export const useProgramData = (programID) => {
    ...
    const { data: programData } = useParsedAccountData(programDataAddress, programDataParser);
