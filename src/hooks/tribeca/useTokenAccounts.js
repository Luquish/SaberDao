...
import { deserializeAccount, getATAAddress, RAW_SOL, TOKEN_PROGRAM_ID, TokenAmount, } from "@saberhq/token-utils";
 ...
 * @returns
 */
export const useTokenAccounts = (address) => {
    ...
    const tokenAccountsRaw = useQuery({
        ...
        queryFn: async () => {
            ...
            return await Promise.all(raw.value.map(async ({ pubkey, account }) => {
    ...
    const tokens = useTokens(useMemo(() => tokenAccountsRaw.data?.map((ta) => ta.mint) ?? [], [tokenAccountsRaw.data]));
    return useQuery({
        queryKey: ["tokenAccounts", network, address?.toString()],
        queryFn: () => {
            invariant(address, "address");
            const sol = nativeBalance.data &&
                ...
                ? {
                    ...
                    balance: new TokenAmount(RAW_SOL[network], nativeBalance.data.accountInfo.lamports),
                    isATA: false,
                }
                : null;
            const result = tokenAccountsRaw.data?.map(({ mint, balance, ...taRest }) => {
