...
const getTwitterImage = async (handle) => {
    const response = await fetch(`https://api.cardinal.so/twitter/proxy?url=https://api.twitter.com/2/users/by&usernames=${handle}&user.fields=profile_image_url`);
    const json = (await response.json());
    return json?.data[0]?.profile_image_url.replace("_normal", "");
};
export const useAddressImage = (address) => {
    const [addressImage, setAddressImage] = useState(undefined);
    const [loadingImage, setLoadingImage] = useState(true);
    const { displayName, loadingName } = useAddressName(address);
    const refreshImage = async (displayName) => {
        if (!displayName) {
    ...
    const theAddressImage = addressImage ??
        mapSome(address, (a) => {
