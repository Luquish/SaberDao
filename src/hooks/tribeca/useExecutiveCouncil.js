...
import { useBatchedSubaccountInfos, useGokiSmartWalletData, } from "../../utils/tribeca/parsers";
...
export const useExecutiveCouncil = () => {
    ...
    const { data: subaccountInfoKeys } = useQuery({
        ...
        queryFn: async () => {
            ...
            return await Promise.all(smartWalletData.account.owners.map(async (swOwner) => {
    ...
    const { data: subaccountInfos } = useBatchedSubaccountInfos(subaccountInfoKeys);
    const subaccountInfo = useMemo(() => subaccountInfos?.find((s) => s && "ownerInvoker" in s.account.subaccountType), [subaccountInfos]);
    ...
    const isMemberOfEC = !!(sdkMut &&
        ecWallet.data?.account.owners.find((o) => o.equals(sdkMut.provider.wallet.publicKey)));
    const buildOwnerInvokeTX = async (tx) => {
    ...
    const ownerInvokeTX = async (...[tx, ...rest]) => {
        return await signAndConfirmTX(await wrapTx(await buildOwnerInvokeTX(tx)), ...rest);
    };
    return {
        ecWallet,
        isMemberOfEC,
