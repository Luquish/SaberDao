...
import { QUARRY_ADDRESSES, findMergeMinerAddress } from '@quarryprotocol/quarry-sdk';
import { chunk } from 'lodash';
import throat from 'throat';
import { Token, TokenAmount, getATAAddressSync } from '@saberhq/token-utils';
...
const calculateUsdValue = (pool, balance, maxSlippagePercent) => {
...
const getStakedBalanceAta = (owner, quarry, lpTokenAddress) => {
...
const getQuarryInfo = async (quarry, rewarders, pools) => {
    // Get quarry info
    ...
    const rewarderInfo = (await Promise.all(chunks.map(throat(10, async (chunk) => {
        const result = await quarry.programs.Mine.account.quarry.fetchMultiple(chunk);
        return result;
    })))).flat();
    // Merge in pool
    pools.forEach(pool => {
    ...
    const replicaInfos = (await Promise.all(poolChunks.map(throat(10, async (chunk) => {
        ...
        const result = await quarry.programs.Mine.account.quarry.fetchMultiple(addressesToFetch);
        return result;
    })))).flat();
        ...
        queryFn: async () => {
            ...
            const data = {
                ...
                pools: pools.pools.map((poolRaw) => {
                    const pool = poolRaw;
                    const swap = swaps.find((s) => s.id === pool.id) ?? null;
                    if (!swap) {
            ...
            await Promise.all(data.pools.map(async (pool) => {
                ...
                if (pool.replicaQuarryData) {
                    secondaryApy = await Promise.all(pool.replicaQuarryData.map(async (replica) => {
                        ...
                        const price = await getPrice(replica.info.redeemer?.tokenInfo.address ?? replica.info.rewardsToken.mint, replica.info.redeemer?.tokenInfo.decimals ?? replica.info.rewardsToken.decimals);
            ...
            if (wallet?.adapter.publicKey) {
                ...
                const legacyMinerBalances = data.pools.map((pool) => {
                    return getStakedBalanceAta(wallet.adapter.publicKey, new PublicKey(pool.info.quarry), new PublicKey(pool.info.lpToken.address));
                });
                ...
                const mergeMinerBalances = await Promise.all(data.pools.map(async (pool) => {
                    ...
                    return getStakedBalanceAta(mmAddress, new PublicKey(pool.info.quarry), new PublicKey(pool.info.lpToken.address));
                }));
                ...
                const tokenAmounts = (await Promise.all(chunks.map(throat(10, async (chunk) => {
                    const result = await connection.getMultipleParsedAccounts(chunk.map((account) => new PublicKey(account.ata)));
