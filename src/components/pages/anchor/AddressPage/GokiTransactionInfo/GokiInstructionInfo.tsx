...
import { fmtObject } from "../../InspectorPage/SimulationSection/AccountDiff/makeDiff";
...
export const GokiInstructionInfo: React.FC<Props> = ({ ix, index }: Props) => {
  const [expanded, setExpanded] = React.useState(false);
  const programId = ix.programId;
  const { data: ixData, accounts } = useParsedProposalInstruction(ix);
  return (
    <Card
      ...
      title={
        <>
          <h3 tw="mb-0 flex items-center gap-2">
            ...
            <InstructionSummary instruction={ix} />
          </h3>
