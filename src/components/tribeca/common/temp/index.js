...
const ownerInvokerSignTX = async (smKey, owner, sdkMut, tx) => {
    const sw = await sdkMut.loadSmartWallet(smKey);
    console.log(sw);
    const allTXs = await Promise.all(tx.instructions.map(async (instruction) => {
...
export const GrantToEC = () => {
    ...
    const run = async () => {
        ...
        const admin = pools
            .filter((pool) => pool.account.manager.toString() === SABER_POOL_MANAGER.toString())
            .map((pool) => poolManager.program.instruction.commitNewAdmin({
            accounts: {
                ...
                newAdmin: new PublicKey("6g57y5PLqJNYKxB4ZGMgVrrxu1jMSr3FYDeL1m8kiwza"),
            },
        }));
        ...
        const beneficiary = poolManager.program.instruction.setBeneficiary({
            accounts: {
                poolManager: poolManager.key,
                admin: new PublicKey("2a3mWszftJ9xkpSmmqZEjhNCjTgCMnXuRcJQHFUJXT9x"),
                beneficiary: new PublicKey("9Nim4EjhtkEwjQtP5KKmHQeXZ9JCznddeFxKgYwMzQAZ"),
            },
        });
        const mintProxy = SABER_CODERS.MintProxy.getProgram(sdkMut.provider);
        const transferOwnershipTX = mintProxy.state.instruction.transferOwnership(new PublicKey("6g57y5PLqJNYKxB4ZGMgVrrxu1jMSr3FYDeL1m8kiwza"), {
            accounts: {
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            if (!chunk)
                continue;
            setCur(i + 1);
            const tx = await ownerInvokerSignTX(new PublicKey("Hq1K3tCMzXVePh4ViNKA12PCcrtye3sqqLRWv79vb8hp"), new PublicKey("F4VFp4tFTyrQWo9YVjCbPE5eQP27ice2zyGDp2tN2Rkm"), sdkMut, TransactionEnvelope.create(provider, chunk));
            const instructions = [
    ...
    return (React.createElement(CardWithImage, { title: "Update Pool Manager", image: React.createElement("div", { className: "flex items-center justify-center h-full" },
            React.createElement(FaSign, { className: "w-20 h-20" })) },
        React.createElement(AsyncButton, { onClick: run }, "Start transactions"),
...
export const Temp = () => {
    return (React.createElement("div", null,
        React.createElement(GovernorProvider, null,
