...
export const GrantToEC: React.FC = () => {
  ...
  const run = async () => {
    ...
    const admin = pools
      .filter(
        (pool: { account: { manager: PublicKey }, publicKey: PublicKey }) =>
            pool.account.manager.toString() === SABER_POOL_MANAGER.toString()
      )
      .map((pool: { account: { manager: PublicKey; swap: PublicKey }, publicKey: PublicKey }) =>
    ...
    const mintProxy = SABER_CODERS.MintProxy.getProgram(sdkMut.provider as any);
...
export const Temp: React.FC<RouteComponentProps> = () => {
  return (
    <div>
      <GovernorProvider>
        <GrantToEC />
