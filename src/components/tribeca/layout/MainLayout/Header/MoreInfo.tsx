...
export const MoreInfo: React.FC = () => {
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-col flex-nowrap p-2 bg-white shadow border rounded dark:(bg-warmGray-850 border-warmGray-800)">
          {MORE_ITEMS.map((item) => (
            <a
              href={item.href}
              type="button"
              ...
              className="space-x-3 text-gray-900 hover:text-primary p-2 font-medium flex items-center appearance-none dark:(text-white hover:text-primary)"
            >
              <div>{item.icon}</div>
              <div>{item.label}</div>
            </a>
