...
export const Nav = ({ className }) => {
    ...
    return (React.createElement("nav", { className: clsx("flex gap-2", className) }, navLinks.map(({ title, href }) => (React.createElement(Link, { key: href, to: href.replace(':governor', governor ?? ''), className: clsx("px-3 py-1 rounded text-sm font-semibold transition-colors", "hover:text-white", "active:text-white active:bg-warmGray-800"), getProps: ({ isCurrent }) => ({
